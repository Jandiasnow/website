...
## 1. 新增应用
首先我们需要在云梯工作台【应用管理】-【新增应用】下创建一个应用，如果您已有可用的应用则可以跳过本步，如下所示，我们创建一个名为 `Hello World` 的空白应用：
...
接下来我们需要在云梯工作台【分支管理】-【创建分支】创建一个开发分支，如果您已有相关分支可以跳过本步，如下所示，我们创建一个名为 `dev` 的分支：
:::info
...
点击【页面管理】，点击【右上角选择分支】切换到 `dev` 分支，如图，

接下来我们需要创建一个页面来实现 `Hello World` 的功能，如下所示，当前我们还没有页面模板（已有的页面），可以不选择：

![创建页面](images/page_create.png)

我们可以通过右上角按钮进行编辑页面信息和删除页面操作，如图：

![编辑页面](images/page_create.png)
...
:::tip
  ...
  |@tenx-ui/charts| 基于 bizcharts 的图表组件资产包 | npm 组件 |
...
:::
  ...
  全局设置主要包括 `antd` 全局样式设置，应用版本设置 等，如下图：

  ![全局设置](images/global_config.png)

  ### 5.3 拖入组件并配置属性

  我们的 `Hello World` 示例非常简单，总共包含三个组件，开发者可以从组件库面板中将组件先拖入画布，然后通过右侧面板配置相关属性：

  刷新按钮- 用于刷新页面数据，我们将组件的 图标属性 设置为关闭，为了美观我们在样式面板中设置下边距：`20px`；
  表格- 用于展示列表数据，我们将表格的 列标题 属性设置为 "应用名称"、 应用id，如下所示；
...
:::
  ...
  1. 首先点击进入【资产配置】面板，添加 `sdk` 资产包 `@tenx-ui/yunti-bff-client`， 如下图:

  ![添加数据源 npm 包](images/assets_npm_bff.png)

  2. 点击进入【全局上下文管理】面板，添加全局函数 `bff`， 如下图:

   ![添加全局函数](images/global_add_func.png)

  3. 绑定 sdk 中的请求方法：
   
     - 方式一：可以通过 `this.utils.bff.getXXX` 在 js 面板中来调用 `sdk` 中的请求方法

     - 方式二：【属性设置面板】-【调用函数】（目前仅支持解析全局函数 `bff` 所绑定的 npm 资产的方法 ），如下图：

    ![添加页面数据](images/page_add_api.png)

  4. 最后一步

  1）我们需要将接口返回的数据变量 `this.props.useGetCurrentUserApps?.data?.currentUser?.apps` 绑定到表格组件的 表格数据 属性上进行展示；

  2）将变量 `this.props.useGetCurrentUserApps?.loading` 绑定到表格组件的 加载中 属性上进行展示，
  
  3）将变量 `this.props.useGetCurrentUserApps?.data?.currentUser?.apps?.length`  绑定到表格组件的 数据总数 属性上进行展示
...
:::
  ...
  打开源码面板，创建一个 `refresh` 方法，用来刷新数据，如下图：

  ![添加页面函数](images/page_add_func.png)

### 5.6 绑定事件
给按钮绑定点击事件，如下所示，选择源码面板中已经创建的 `refresh` 方法，如果不选择，绑定后我们会根据低码的事件模板在 js 面板自动生成一个 JS 函数： 
...
最后可以通过【应用发布】模块来发布应用，详见 发布一个 `Hello World` 组件，或者点击【出码】按钮进行 `ProCode` 出码，如下图所示
